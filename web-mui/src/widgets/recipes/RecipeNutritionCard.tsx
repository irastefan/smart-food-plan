    ...
    >
      <Stack spacing={compact ? 0.75 : 2.5} sx={{ minHeight: "100%", justifyContent: "space-between" }}>
        ...
        <Stack direction="row" spacing={compact ? { xs: 0.75, sm: 1 } : { xs: 1.5, sm: 2, md: 3 }} alignItems="center" sx={{ flex: 1 }}>
          <Stack direction="row" spacing={compact ? 0 : { xs: 0.5, sm: 1 }} alignItems="center" sx={{ minWidth: compact ? { xs: 70, sm: 78 } : { xs: 88, sm: 124, md: 160 }, flexShrink: 0 }}>
            <MacroCalorieRing
          ...
          <Stack direction="row" spacing={compact ? { xs: 0.8, sm: 1 } : { xs: 1.25, sm: 1.75, md: 2.75 }} useFlexGap sx={{ flex: 1, minWidth: 0 }}>
            {macroItems.map((item) => (
              <Stack key={item.key} spacing={compact ? 0.2 : { xs: 0.35, sm: 0.6, md: 0.75 }} sx={{ minWidth: 0, flex: 1 }}>
                ...
                <Typography sx={{ fontSize: compact ? { xs: 12, sm: 14 } : { xs: 14, sm: 18, md: 22 }, lineHeight: 1, fontWeight: 800, whiteSpace: "nowrap" }}>
                  {item.value.toFixed(1)} g
                </Typography>
                <Typography color="text.secondary" sx={{ fontWeight: 700, fontSize: compact ? { xs: 9, sm: 10 } : { xs: 10, sm: 12, md: 14 } }}>
                  {item.label}
                </Typography>
              </Stack>
            ))}
  ...
  return (
    <Box sx={{ position: "relative", width: compact ? size : { xs: size, md: 120 }, height: compact ? size : { xs: size, md: 120 }, display: "grid", placeItems: "center", flexShrink: 0 }}>
      ...
      <Stack spacing={0.15} alignItems="center" sx={{ position: "absolute", inset: 0, justifyContent: "center" }}>
        <Typography sx={{ fontSize: compact ? { xs: 20, sm: 22 } : { xs: 24, md: 34 }, lineHeight: 1, fontWeight: 800 }}>
          {calories}
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontWeight: 600, fontSize: compact ? { xs: 10, sm: 11 } : { xs: 12, md: 14 } }}>
          kcal
        </Typography>
      </Stack>
    </Box>
